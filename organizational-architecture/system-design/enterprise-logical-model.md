...
A logical attribute name (e.g., `sla_category`) is a legitimate logical-layer concept. Its eventual column type, index strategy, and storage representation are physical-layer concerns belonging to the SQL Foundation phase that follows.
...
|**Planned implementation through an existing governed pattern**|1|Workforce Escalation → Ticket (reuses `related_ticket_id`)|
...
## Provisional Identifier Governance

Any canonical identifier not already present in the Cross-System Identifier Dictionary is marked **Provisional — Pending Governance Approval**. Renames of existing identifiers (e.g., `schedule_id` → `coverage_schedule_id`) route through the same identifier-governance review as net-new identifiers, per governance decision — the prior field name is documented as a legacy source-field alias during migration, not silently replaced.
...
This replaces a two-tier canonical/subsystem-local split used in an earlier draft, which risked the physical SQL phase reading "not enterprise-shared" as "safe to omit."
...
* **Governed Relationships:** Location Inventory; Inventory Item indirectly through Location Inventory; Shipment, Replenishment, Shortage, Inventory Discrepancy, and Ticket
...
* **Governed Relationships:** Vendor (preferred-sourcing only), Shipment, Replenishment, Location Inventory, Inventory Discrepancy, Shortage
...
```text
  ...
  This sequence is in scope for the SQL Foundation initiative but should be tracked as a separate workstream within it. Strict FK enforcement should not be enabled before existing values are mapped and validated.
...
* **Governed Relationships:**
  ...
  * Inventory Discrepancy, Shortage, Replenishment, Shipment, Corrective Action, SLA Event — via the *Established* `related_ticket_id` pattern

---

### Assignment

* **Canonical Identifier:** `assignment_id` — **Provisional — Pending Governance Approval**
...
* **Uniqueness Rule:** Deferred pending governed assignment categorization.
...
* **Governed Relationships:** Employee, Ticket and Corrective Action *(via required associative entities)*, Coverage Schedule *(deferred)*, Workload Record *(analytical/historical)*, Workforce Escalation

---

### Corrective Action

* **Canonical Identifier:** `corrective_action_id` — **Provisional — Pending Governance Approval**
...
* **Uniqueness Rule:** `corrective_action_id` unique. Duplicate-detection heuristics (vendor + issue type + source event + open date) may later serve as guidance, not as a key.
...
* **Logical Attributes — Lifecycle State:** `corrective_action_status` [Enterprise-Shared]; `corrective_action_severity` [Domain-Authoritative]
* **Logical Attributes — Source References:** `sla_event_id` (FK, optional), `fulfillment_event_id` (FK, optional) [Enterprise-Shared]
* **Logical Attributes — Domain Detail:** `corrective_action_type`, `trigger_reason`, `assigned_owner`, `remediation_plan_summary`, `monitoring_start_date`, `reassessment_date`, `recovery_status` [Domain-Authoritative]
* **Logical Attributes — Descriptive:** `notes` [Descriptive-or-Derived]
* **Governed Relationships:** Vendor, SLA Event, Fulfillment Event, Ticket (via `related_ticket_id`), Employee *(planned, via `assigned_owner` formalization)*
...
* **Logical Attributes — References:** `vendor_id`, `item_id`, `location_id` (FKs) [Enterprise-Shared]
...
* **Governed Relationships:** Vendor, Inventory Item (single-item, current scope), Location, Replenishment *(via required associative entity)*, Fulfillment Event, SLA Event, Shortage, Ticket (via `related_ticket_id`)
...
* **Logical Attributes — References:** `item_id`, `location_id`, `vendor_id` (FKs) [Enterprise-Shared]
...
* **Canonical Identifier:** `location_inventory_id` — Present in current repository data; **Provisional — Pending Governance Approval** for cross-system status
...
* **Canonical Identifier:** `coverage_schedule_id` — **Provisional — Pending Governance Approval** (legacy source field: `schedule_id`, to be documented as a migration alias, not silently replaced)
...
* **Integrity Rule (distinct from uniqueness):** An Employee should not have incompatible overlapping coverage periods — a temporal-overlap constraint enforced as validation logic, not a uniqueness index.
...
* **Canonical Identifier:** `workload_record_id` — **Provisional — Pending Governance Approval** (legacy source field: `workload_id`, to be documented as a migration alias)
...
* **Canonical Identifier:** `fulfillment_event_id` — **Provisional — Pending Governance Approval**
...
* **Logical Attributes — References:** `vendor_id`, `shipment_id`, `item_id`, `location_id` (FKs) [Enterprise-Shared]
* **Logical Attributes — Status:** `fulfillment_status` [Enterprise-Shared]
...
* **Canonical Identifier:** `sla_event_id` — **Provisional — Pending Governance Approval**
...
* **Logical Attributes — References:** `vendor_id`, `shipment_id`, `fulfillment_event_id` (FKs) [Enterprise-Shared]
...
* **Canonical Identifier:** `escalation_id` — **Provisional — Pending Governance Approval**
...
* **Known Limitation Carried Forward:** No `employee_id` or `related_ticket_id` attribute currently exists on this entity. Adding `related_ticket_id` — following the same *Established* pattern used by six other objects — is implementation scope this model defines.
* **Governed Relationships:** Employee *(planned, not yet represented)*, Assignment, Coverage Schedule *(both deferred — evidentiary)*, Ticket *(via `related_ticket_id`, to be added)*
...
### Shipment Replenishment Fulfillment

Resolves: Shipment → fulfills → Replenishment — documented as one of the most significant architectural gaps identified during Phase 3.

* `shipment_id` (FK to Shipment)
* `replenishment_id` (FK to Replenishment)
* `fulfilled_quantity` — **logical requirement, not a physical-schema deferral.** Without it, the model can state that a Shipment fulfills a Replenishment but not what portion — which defeats the stated purpose of this associative entity (supporting partial/multi-shipment fulfillment). Its data type, constraints, and storage are physical-design decisions; its existence is not.
* `fulfillment_date` — **not included.** No current business evidence distinguishes relationship-level fulfillment timing from the Shipment's own receipt date. Add only if that distinction becomes a real requirement.
...
**Workforce Escalation → may generate → Ticket.** Resolved via the *Established* `related_ticket_id` pattern already governing six other relationships. Requires adding `related_ticket_id` to Workforce Escalation, which does not currently exist on that entity.
...
* **Identifier renames** (`schedule_id` → `coverage_schedule_id`, `workload_id` → `workload_record_id`) require the same identifier-governance approval as net-new identifiers. Prior field names are documented as legacy source-field aliases for migration purposes, not silently replaced.
...
* **Ticket free-text migration** (`requesting_location`, `assigned_owner`) is in scope for the SQL Foundation initiative as a distinct data-reconciliation and migration workstream — see the sequence documented under Ticket, above. Referential-integrity enforcement is deferred until after reconciliation completes.

No new open questions from this revision.

---

# Summary

This document establishes the Enterprise Logical Model derived from the approved Enterprise Object Model and Enterprise Relational Model. It resolves 4 associative entities as genuinely required by business meaning, reuses 1 existing governed pattern, and explicitly defers 6 relationships and 2 analytical/historical relationships with documented reasoning.

Business candidate keys are stated as business conclusions, never as the canonical identifier restated, and never justified solely by a data observation. Attribute provenance uses a three-part distinction (Enterprise-Shared / Domain-Authoritative / Descriptive-or-Derived) specifically to prevent the SQL Foundation phase from misreading domain-important attributes as optional.

8 of 17 entities require provisional canonical identifiers. None have been written back to the Cross-System Identifier Dictionary. A dedicated identifier-governance review follows approval of this document, before physical SQL design begins.
